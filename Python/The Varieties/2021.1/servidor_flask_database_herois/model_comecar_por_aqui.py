...
abaixo
'''
from herois import HeroiNaoExisteException, consultar_heroi_por_nome, criar_heroi_banco
import herois
'''
Parte 1: Consultas
...
'''
from itens import ItemNaoExisteException, nome_para_id_item, colocar_item_em_uso_banco
import itens
'''
Ex3
O arquivo itens.py
...
def heroi_pronto_por_nome(nomeHeroi):
    ...
    itens = itens_em_uso_por_nome_do_heroi(nomeHeroi)
    heroi['vida'] = heroi['fisico'] * 10
    if itens:
        for item in itens:
            heroi['magia'] += item[1]
            heroi['fisico'] += item[2]
            heroi['agilidade'] += item[3]
    return heroi
'''
...
def atacar_com_fisico(atacante, defensor):
    ataques = atacante['agilidade'] // defensor['agilidade']
    if ataques < 1:
...
'''
from itens_do_heroi import itens_em_uso_do_heroi, itens_em_uso_por_nome_do_heroi, dar_item_para_heroi_banco
...
'''
def lista_itens_em_uso_do_heroi(idHeroi):
    return itens_em_uso_do_heroi(idHeroi)
'''
Ex12
...
'''
def criar_heroi(nome,agilidade,fisico,magia):
    criar_heroi_banco(nome, agilidade, fisico, magia)
    soma_atributos = agilidade + fisico + magia
    if soma_atributos > 20: raise OverpowerException
...
def dar_item_para_heroi(heroi, item):
    dar_item_para_heroi_banco(heroi['id'], item['id'])
...
def colocar_item_em_uso(heroi, item):
    itens_em_uso = itens_em_uso_do_heroi(heroi['id'])
    if not itens_em_uso:
        colocar_item_em_uso_banco(heroi['id'], item['id'])
    else:
