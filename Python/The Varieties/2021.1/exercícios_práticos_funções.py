...
'''
EXEMPLO: Observe a função 'soma', implementada a seguir.
'''
...
def soma_dos_quadrados(a, b):
    soma_a = a ** 2
    soma_b = b ** 2
    return soma_a + soma_b   # return quadrado(a) + quadrado(b)    
...
def soma_divisores(n):
    soma = 0
    for numero_divisor in range(1, n+1):
        if n % numero_divisor == 0:
            soma += numero_divisor
    return soma
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_06_calcular_salario(self):
    ...
    def test_10_soma_divisores(self):
