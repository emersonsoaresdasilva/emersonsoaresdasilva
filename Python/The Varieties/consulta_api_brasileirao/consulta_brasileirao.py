...
def pega_dados():
    with open('consulta_brasileirao.json') as f:
        dados = json.load(f)
    return dados
...
#print(dados2018['fases']['2700']['classificacao']['grupo']['Único'][:5])
#print(dados2018['fases']['2700']['faixas-classificacao']['classifica1'])

# Funçoes Auxiliares.
def equipes_na_tabela(dados):
    return dados['fases']['2700']['classificacao']['grupo']['Único']

def pegar_equipes(dados):
    return dados['equipes'].values()

def pegar_jogos(dados):
    return dados['fases']['2700']['jogos']['id']
...
def id_campeao(dados):
    return equipes_na_tabela(dados)[0]
...
def qtos_libertadores(dados):
    ...
    # Caso haja possibilidade da faixa inicial ser maior que 1, o código abaixo funciona:
    ''' 
    faixa_inicial, faixa_final = dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa'].split('-')
    return int(faixa_final) - int(faixa_inicial) + 1 
    '''
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    return equipes_na_tabela(dados)[:numero_de_times]
...
def nomes_classificados_libertadores(dados):
    nomes_times = []
    id_times = classificados_libertadores(dados)
    for id in id_times:
...
def ids_dos_times_de_um_jogo(dados,id_jogo):
    jogo = pegar_jogos(dados)[id_jogo]
...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
    ...
    time_casa = nome_do_time(dados, id_times[0])
    time_visita = nome_do_time(dados, id_times[1])
    return time_casa, time_visita
...
def dicionario_id_estadio_e_nro_jogos(dados):
    estadio_jogos = {}
    jogos = pegar_jogos(dados).values()
    for jogo in jogos:
        estadio = jogo['estadio_id']
        if estadio in estadio_jogos:
            estadio_jogos[estadio] += 1
        else:
            estadio_jogos[estadio] = 1
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    times_encontrados = []
    for time in pegar_equipes(dados):
        strings = [time['nome-comum'], time['nome-slug'], time['sigla'], time['nome']] 
        for string in strings:
            if nome_time in string:
                times_encontrados.append(time['id'])
...
def ids_de_jogos_de_um_time(dados,time_id):
    jogos_do_time = []
    id_jogos = pegar_jogos(dados).keys()
    for id in id_jogos:
        if time_id in ids_dos_times_de_um_jogo(dados, id):
            jogos_do_time.append(id)
    return jogos_do_time
...
def dicionario_de_gols(dados):
    lista_equipes = dados['equipes'].keys()
    dicio_equipes = dict((id,0) for id in lista_equipes)  #Transformando lista em dicionário
    for time in lista_equipes:
        ids_jogos = ids_de_jogos_de_um_time(dados, time)
        for id in ids_jogos:
            ids_times = ids_dos_times_de_um_jogo(dados, id)
            if ids_times[0] == time:
                gols = int(pegar_jogos(dados)[id]['placar1'])
            else:
                gols = int(pegar_jogos(dados)[id]['placar2'])
            dicio_equipes[time] += gols 
    return dicio_equipes
...
def time_que_fez_mais_gols(dados):
    times_gols = dicionario_de_gols(dados)
    times_gols = (dict(sorted(times_gols.items(), key=lambda item: item[1]))) #Ordena times por gols
    return list(times_gols.keys())[-1]
...
def rebaixados(dados):
    inicio, fim = dados['fases']['2700']['faixas-classificacao']['classifica3']['faixa'].split('-')
    inicio = int(inicio)-1
    fim = int(fim)
    return equipes_na_tabela(dados)[inicio:fim]
...
def classificacao_do_time_por_id(dados,time_id):
    ids_melhores = ids_dos_melhor_classificados(dados, -1)
    if time_id in ids_melhores:
        return ids_melhores.index(time_id) + 1
    return 'nao encontrado'
